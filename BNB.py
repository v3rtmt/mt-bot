...
def bnb():
	...
	ticker = binance.fetch_ticker('BNB/BUSD')
...
@BNB.route('/Price-BNB', methods=['POST'])
def Price():
	@app.after_response
	def afterPrice():
		operationFilter = {"Operation-BNB": True}	
...
@BNB.route('/Script-BNB', methods=['POST'])
def script():
	status = mongo.db.Status
...
def createOrders():
	...
	if thisBot['exchange'] == "Binance":
		...
		binance.enableRateLimit = True
		binance.set_leverage(thisBot['quantityLeverage'], 'BNB/BUSD', params={"marginMode": "isolated"})
		...
		def createLimitOrderBinance():
			...
			try:
				if Operation['side'] == "BUY":
					order = binance.create_limit_buy_order('BNB/BUSD', tradeAmount, orderPrice)
					thisOrder = True
				elif Operation['side'] == "SELL":
					order = binance.create_limit_sell_order('BNB/BUSD', tradeAmount, orderPrice)
					thisOrder = True
				else:
			...
			if (thisOrder == True) and (order['status'] == "open"):
				...
				retrying = True
				thisOrder = False
				binance.cancel_all_orders('BNB/BUSD')
	...
	#-------------------- BYBIT --------------------
		...
		bybit.set_leverage(thisBot['quantityLeverage'], 'BNB/BUSD', params={"marginMode": "isolated"})
		...
		def createLimitOrderBybit():
			...
			ticker = bybit.fetch_ticker('BNB/BUSD')
			orderPrice1 = float(ticker['close'])
			orderPrice = orderPrice1
			tradeAmount = ( ( thisBot['tradeAmount'] * thisBot['quantityLeverage'] ) / orderPrice )
			...
			try:
				if Operation['side'] == "BUY":
					order = bybit.create_limit_buy_order('BNB/BUSD', tradeAmount, orderPrice)
					thisOrder = True
				elif Operation['side'] == "SELL":
					order = bybit.create_limit_sell_order('BNB/BUSD', tradeAmount, orderPrice)
					thisOrder = True
				else:
					print("ERROR SIDE (SIDE NO DECLARADA) [EXCHANGE]")
			except:
			...
			if (thisOrder == True) and (order['status'] == "open"):
				...
				retrying = True
				thisOrder = False
				bybit.cancel_all_orders('BNB/BUSD')
...
def cancelOrders():
	...
	if thisBot['exchange'] == "Binance":
		...
		def createMarketOrderBinance():
			...
			try:
				if Operation['side'] == "BUY":
					order = binance.create_market_sell_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
					issues = "None"
				elif Operation['side'] == "SELL":
					order = binance.create_market_buy_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
					issues = "None"
				else:
					print("ERROR SIDE (SIDE NO DECLARADA) [EXCHANGE]")
					issues = "Invalid data"
	...
	#-------------------- BYBIT --------------------
		...
		def createMarketOrderBybit():
			...
			try:
				if Operation['side'] == "BUY":
					order = bybit.create_market_sell_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
					issues = "None"
				elif Operation['side'] == "SELL":
					order = bybit.create_market_buy_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
					issues = "None"
				else:
					print("ERROR SIDE (SIDE NO DECLARADA) [EXCHANGE]")
					issues = "Invalid data"
	...
	if thisBot['exchange'] == "Binance":
		...
		def checkMarketOrderBinance():
			...
			if balanceBUSD < thisBot['tradeAmount']:
				...
				try:
					if Operation['side'] == "BUY":
						binance.create_market_sell_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
						issues = "Limit Balance Reached"
					elif Operation['side'] == "SELL":
						binance.create_market_buy_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
						issues = "Limit Balance Reached"
					else:
						print("ERROR SIDE (SIDE NO DECLARADA) [EXCHANGE]")
						issues = "Invalid data"
	...
	#-------------------- BYBIT --------------------
		...
		def checkMarketOrderBybit():
			...
			if balanceBUSD < thisBot['tradeAmount']:
				...
				try:
					if Operation['side'] == "BUY":
						bybit.create_market_sell_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
						issues = "Limit Balance Reached"
					elif Operation['side'] == "SELL":
						bybit.create_market_buy_order('BNB/BUSD', thisBot['lastOrderAmount'], params={'reduce_only': True})
						issues = "Limit Balance Reached"
					else:
						print("ERROR SIDE (SIDE NO DECLARADA) [EXCHANGE]")
						issues = "Invalid data"
